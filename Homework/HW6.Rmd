...
```{r}
(distance_range <- range(flights$distance))
(distance_mean <- mean(flights$distance))
(distance_median <- median(flights$distance))
```
...
```{r}
ggplot(airline_flights, aes(month, dep_delay)) +
  geom_point() +
  scale_x_continuous(n.breaks = 12) +
  stat_summary(fun= "mean", geom = "point", fill = "red", pch = 23, size = 2)
```
